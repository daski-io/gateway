ALTER TABLE bazaar_listing_bindings
  ADD COLUMN fulfillment_signer BYTEA NOT NULL
    CHECK (octet_length(fulfillment_signer) = 20),
  ADD CONSTRAINT bazaar_listing_bindings_fulfillment_role_check CHECK (
    fulfillment_signer <> pay_to
  );

CREATE TABLE bazaar_provider_key_roles (
  key_address BYTEA PRIMARY KEY CHECK (octet_length(key_address) = 20),
  key_role TEXT NOT NULL CHECK (key_role IN ('provider', 'fulfillment')),
...
CREATE TABLE bazaar_fulfillment_attestations (
  ...
  evidence_id BYTEA NOT NULL UNIQUE CHECK (octet_length(evidence_id) = 32),
