...
export class BazaarRuntimeDrainPendingError extends Error {
  constructor() {
    super("Bazaar runtime manifest transition is draining live work");
  }
}
...
export async function activateBazaarRuntimeWiring(input: {
  ...
  const transition = await transitionBazaarRuntimeManifest({
    pool: input.pool,
    identity: unsignedIdentity,
    approval: input.wiring.runtimeManifestApproval,
    trust,
    wiring: input.wiring,
    reconcile: async (client, identity) => {
  ...
  if (transition.state === "draining") throw new BazaarRuntimeDrainPendingError();
