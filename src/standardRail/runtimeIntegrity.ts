...
export async function measureRuntimeIntegrity(
  ...
  const gatewayKeys = [
    ...
    ["reputation-order", rail.reputationOrderPrivateKey],
    ["reputation-relayer", rail.reputationRelayerPrivateKey],
    ["reputation-mirror", rail.mirror.privateKey],
    ["order-event", rail.notification.privateKey],
  ].map(([role, key]) => ({
    role,
  ...
  return {
    ...
    canonicalConfigurationHash: canonicalHash({
      ...
      marketplaceCommissionBps: rail.marketplaceCommissionBps,
      launchOutcomeIds: [...rail.launchOutcomeIds].sort(),
      reviewedListings: rail.reviewedListings,
      ...
      cursorPolicy: {
        activeKeyId: rail.cursorKeyRing.activeKeyId,
        keys: [...rail.cursorKeyRing.keys.entries()].map(([keyId, value]) => ({
          keyId,
          keyHash: keccak256(bytesToHex(value)),
        })).sort((left, right) => left.keyId.localeCompare(right.keyId)),
      },
      reputation: {
        contract: rail.reputationContract.toLowerCase(),
        deploymentBlock: rail.reputationDeploymentBlock.toString(),
        easAddress: rail.easAddress.toLowerCase(),
        outcomeSchemaUid: rail.reputationOutcomeSchemaUid.toLowerCase(),
        confirmationSchemaUid: rail.reputationConfirmationSchemaUid.toLowerCase(),
        retryDelaysSeconds: rail.reputationRetryDelaysSeconds,
        permitTtlSeconds: rail.reputationPermitTtlSeconds,
        maxFeePerGasWei: rail.reputationMaxFeePerGasWei.toString(),
        maxPriorityFeePerGasWei: rail.reputationMaxPriorityFeePerGasWei.toString(),
        registerGasLimit: rail.reputationRegisterGasLimit.toString(),
        confirmationGasLimit: rail.reputationConfirmationGasLimit.toString(),
        refundGasLimit: rail.reputationRefundGasLimit.toString(),
        minimumReserveWei: rail.reputationMinimumReserveWei.toString(),
      },
      confirmation: {
        deadlineSeconds: rail.confirmationDeadlineSeconds,
        maxPerOrder: rail.confirmationMaxPerOrder,
        maxPerPayerPerDay: rail.confirmationMaxPerPayerPerDay,
        maxGlobalPerDay: rail.confirmationMaxGlobalPerDay,
      },
      mirror: {
        registry: rail.mirror.registry.toLowerCase(),
        maxTransactionsPerOrder: rail.mirror.maxTransactionsPerOrder,
        maxTransactionsPerUtcDay: rail.mirror.maxTransactionsPerUtcDay,
        maxFeePerGasWei: rail.mirror.maxFeePerGasWei.toString(),
        maxPriorityFeePerGasWei: rail.mirror.maxPriorityFeePerGasWei.toString(),
        giveGasLimit: rail.mirror.giveGasLimit.toString(),
        revokeGasLimit: rail.mirror.revokeGasLimit.toString(),
        minimumReserveWei: rail.mirror.minimumReserveWei.toString(),
      },
      notification: {
        keyId: rail.notification.keyId,
        previousKeys: rail.notification.previousKeys,
        verificationTimeoutMs: rail.notification.verificationTimeoutMs,
        verificationTtlSeconds: rail.notification.verificationTtlSeconds,
        maxResponseBytes: rail.notification.maxResponseBytes,
        maxPendingPerPayer: rail.notification.maxPendingPerPayer,
        maxPendingGlobal: rail.notification.maxPendingGlobal,
        retryDelaysSeconds: rail.notification.retryDelaysSeconds,
        maxAttempts: rail.notification.maxAttempts,
      },
      abuse: rail.abuse,
      objectStore: {
    ...
    keyPolicySetHash: canonicalHash({
      ...
      cursorKeys: [...rail.cursorKeyRing.keys.entries()].map(([keyId, key]) => ({
        keyId,
        keyHash: keccak256(bytesToHex(key)),
      })).sort((left, right) => left.keyId.localeCompare(right.keyId)),
      notificationPreviousKeys: rail.notification.previousKeys,
    }),
