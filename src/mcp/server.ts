    ...
    server.registerTool(
      "daski_get_task_status",
      {
        description: [
          ...
          "- ADVANCED — skip that first-poll error entirely: for reads you may CONSTRUCT the authorization yourself instead of waiting for the challenge. Sign typed-data with message `{ buyerTokenId, taskId, action: \"get\", nonce: <any fresh 0x + 64-hex-char value>, expiry: <unix seconds, at most 15 min ahead> }`, types `TaskAccessAuthorization { buyerTokenId: uint256, taskId: string, action: string, nonce: bytes32, expiry: uint256 }`, and the domain copied VERBATIM from your envelope-auth typed-data (same name/version/chainId/verifyingContract) — then pass it as `capability` on the very first poll. Read-style `action: \"get\"` never burns nonces, so a made-up nonce is fine and the same signed capability stays reusable until expiry. If a self-built capability is rejected (BAD_SIGNATURE / FIELD_MISMATCH), fall back to signing the challenge from the -32107 error — that path always works.",
        ].join("\n"),
        inputSchema: {
          providerA2AUrl: z.string(),
          taskId: z.string(),
    ...
    server.registerTool(
      "daski_buy_service",
      {
        description: [
          ...
          "",
          "Inputs:",
          "- First call (no signature): `providerTokenId`, `skillId`, `serviceArgs` (per the skill's `requiredFields`), `walletAddress`; optional `name` — on a fresh wallet this becomes the permanent display name of the agentId minted with the purchase. If the wallet MIGHT be fresh (no confirmed agentId), pass `name` on this VERY FIRST call: an already-registered wallet ignores it with a harmless `name was ignored` warning, while adding it after seeing `atomic: true` forces a second quote. Decide it BEFORE signing either typed-data block. Renames are unsupported; omit `name` on later purchases from an already-registered wallet.",
